...
from telegram import Update, InputMediaPhoto, InputPaidMedia
...
async def handle_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat_type == 'private':  # Restrict to private chats
        if 'send' in text:
            ...
            # Create InputPaidMedia for the photo (required for paid media)
            ...
            try:
                await context.bot.send_paid_media(
                    chat_id=chat_id,
                    media=[paid_media],  # Must be a list of InputPaidMedia (even for one)
